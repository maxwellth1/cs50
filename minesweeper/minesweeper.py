...
class Sentence():
    ...
    def inference(self, other): 
        ...
        if self.cells.issubset(other.cells):
            diffCells = other.cells - self.cells
            diffCount = other.count - self.count

            if diffCount < 0:
                return  None
            
            return Sentence(diffCells, diffCount)
        
        
        elif other.cells.issubset(self.cells):
            diffCells = self.cells - other.cells
            diffCount = self.count - other.count
...
class MinesweeperAI():
    ...
    def add_knowledge(self, cell, count):
        ...
        #add cells to neighbor cells into new cell set
        ...
        #mark mines and safes in new sentence
        for safeCell in self.safes:
            newSentence.mark_safe(safeCell)

        for mineCell in self.mines:
            newSentence.mark_mine(mineCell)
        ...
        #mark them
        for cell in addSafes:
            self.mark_safe(cell)

        for cell in addMines:
            self.mark_mine(cell)
        ...
        for s1, s2 in itertools.combinations(self.knowledge, 2):
            ...
            if len(inf.cells) > 0 and inf not in self.knowledge and inf not in newSentence:
                newSentence.append(inf)
    ...
    def make_safe_move(self):
        ...
        """
        avaliableMoves = self.safes.difference(self.moves_made)

        if len(avaliableMoves) != 0:
            return random.choice(tuple(avaliableMoves))
